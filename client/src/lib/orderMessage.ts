import { formatAED, type CatalogProduct, type CatalogVariant } from "@/lib/catalog";
...
type OrderLine = { product: CatalogProduct; variant?: CatalogVariant; quantity: number };

export function buildOrderWhatsAppMessage(copy: CommerceCopy, lines: OrderLine[], subtotal: number | null) {
  const items = lines.map(({ product, variant, quantity }) => `- ${product.name}${variant ? ` (${copy.flavor}: ${variant.name})` : ""} x ${quantity} — ${product.priceAED === null ? copy.priceToConfirm : `AED ${(product.priceAED * quantity).toFixed(2)}`}`).join("\n") || "- —";
  const total = subtotal === null ? copy.priceToConfirm : `AED ${subtotal.toFixed(2)}`;
  return [
    "Place Your Order",
    "",
    ...lines.map(({ product, variant, quantity }) => `${product.name}${variant ? ` (${copy.flavor}: ${variant.name})` : ""} x ${quantity} — ${product.priceAED === null ? copy.priceToConfirm : `AED ${(product.priceAED * quantity).toFixed(2)}`}`),
    "",
...
export function buildWhatsAppOrderMessage(copy: CommerceCopy, fields: CheckoutFields, lines: OrderLine[], subtotal: number | null) {
  const items = lines.map(({ product, variant, quantity }) => `- ${product.name}${variant ? ` (${copy.flavor}: ${variant.name})` : ""} x ${quantity} — ${formatAED(product.priceAED).replace("Price to confirm", copy.priceToConfirm)}`).join("\n") || "- —";
  const total = subtotal === null ? copy.priceToConfirm : `AED ${subtotal.toFixed(2)}`;
