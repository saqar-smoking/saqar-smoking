...
export default function OrderPage() {
  ...
  const lines = product
    ...
    : cartProducts;
  const orderSubtotal = product
    ? product.priceAED === null
      ? null
      : product.priceAED * lines[0]!.quantity
    : subtotal;
        ...
        <section className="commerce-frame order-page">
          <div className="order-page__header">
            <div>
              ...
              <h1>
                Place Your Order<em>.</em>
              </h1>
            </div>
            <p>
              Online payment is currently unavailable. Please place your order
          ...
          <div className="order-layout">
                ...
                <div className="order-list">
                  {lines.map(({ product, variant, quantity }) => (
                    <article
                      ...
                      key={`${product.id}-${variant?.id ?? "base"}-${quantity}`}
                    >
                      <img src={product.image} alt={product.name} />
                      <div className="order-item__content">
                        <small>{product.brand}</small>
                        <h3>{product.name}</h3>
                        {variant ? <span>{t.flavor}: {variant.name}</span> : null}
            ...
            <aside className="order-panel order-panel--contact">
              ...
              <div className="order-contact-grid">
                <a
                  className="order-contact-card order-contact-card--whatsapp"
                  href={whatsappHref}
                  target="_blank"
                  rel="noreferrer"
                >
                ...
                <a
                  className="order-contact-card order-contact-card--instagram"
                  href={INSTAGRAM_URL}
                  target="_blank"
                  rel="noreferrer"
                >
                ...
                <a
                  className="order-contact-card order-contact-card--phone"
                  href={PHONE_URL}
                >
                  <Phone size={18} />
                  <span>Phone</span>
              ...
              <div className="order-note">
                <p>
                  We will confirm the final details with you once you contact
                  our team.
                </p>
              </div>
            </aside>
