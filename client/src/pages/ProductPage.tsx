...
import { ArrowLeft, Check, ChevronDown, Instagram, MessageCircle, Minus, Phone, Plus, ShoppingBag } from "lucide-react";
import { Link, useRoute } from "wouter";
import { AgeGateGuard, CommerceHeader, INSTAGRAM_URL, WHATSAPP_URL, PHONE_URL } from "@/components/CommerceShell";
import { fetchCatalogProducts, formatAED, type CatalogProduct } from "@/lib/catalog";
...
export default function ProductPage() {
  ...
  useEffect(() => {
    ...
    void fetchCatalogProducts().then((products) => {
      setProduct(products.find((item) => item.id === params.id && !item.archived));
  ...
  if (!catalogLoaded) return <AgeGateGuard><div className="commerce-shell"><CommerceHeader /><div className="commerce-empty-page"><h1>Loading...</h1></div></div></AgeGateGuard>;
